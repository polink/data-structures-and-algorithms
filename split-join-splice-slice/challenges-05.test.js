...
const listFoods = (recipe) => {
  ...
  // let breakPoint = ' ';
  // let newArr = [];
  // // slice each position on each space, make that into an array, return last result
  // for (var i = 0; i<recipe.ingredients; i++){
  //   let str = recipe.ingredients[i];
  //   str = str.slice(str.indexOf(breakPoint));
  //   // str = str.indexOf(' ');
  //   // str = str.slice();
  //   console.log(str);
  //   newArr.push(str);
  //   // recipe.ingredients[1].slice(0).push(result);
  // }
  // for (let i in newArr){
  //   let newStr = newArr[i];
...
const stepActions = (recipe) => {
  ...
  for (let i in recipe.steps){
    arr.push(recipe.steps[i]);
    arr = arr.shift();
    // let arr1 = (arr[i]).shift('');
    result.push(arr);
  }
  return result;
}
